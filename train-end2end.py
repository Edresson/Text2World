...
class Graph:
    def __init__(self, num=1,mode="train"):
        ...
        if training:
            ...
            self.loss_WSR = tf.losses.mean_squared_error(self.Z , self.worlds_WSR)
            self.loss_worlds1 = tf.losses.mean_squared_error(self.Y , self.worlds)
            self.loss = self.loss_WSR+self.loss_att+self.loss_worlds1
