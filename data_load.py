    ...
    with tf.device('/cpu:0'):
        ...
        if hp.prepro:
            def _load_spectrograms(fpath):
                ...
                worlds = np.array(np.load(world))
                world_wsr = "worlds_wsr/{}".format(fname.replace("wav", "npy"))
                worlds_wsr = np.array(np.load(world_wsr))
        ...
        fname.set_shape(())
        text.set_shape((None,))
        world.set_shape((None, hp.num_bap+hp.num_lf0+hp.num_mgc+hp.num_vuv))
        world_wsr.set_shape((None, hp.num_bap+hp.num_lf0+hp.num_mgc+hp.num_vuv))
        # Batching
